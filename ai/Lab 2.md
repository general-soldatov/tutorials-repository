...
MSE of polymomial regression {mse_poly}
""")
```
Создайте данные для двух новых объектов и выполните предсказание. Так как каждый объект недвижимости характеризуется восемью параметрами, то для каждого объекта недвижимости укажите 8 чисел.
...
```py
# prediction for new real estate objects
...
print(f"""Predict prices LR: {LR.predict(X_new)}
Predict prices PR: {LR_poly.predict(poly.transform(X_new))}
""")
```
Сохраните блокнот через меню «Файл / Сохранить».  
Переименуйте файл на Google Диске, используя шаблон «Лабораторная работа № 2 студента группы … Фамилия Имя», указав свой номер группы, фамилию и имя.
